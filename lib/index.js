...
Intercom.prototype.identify = function(identify, fn){
  ...
  var self = this;
  var collectContext = this.settings.collectContext;
  ...
  this.lock(key, function(err){
    ...
    return self
      ...
      .type('json')
      .accept('json')
      .send(mapper.identify(identify, { collectContext: collectContext }))
...
Intercom.prototype.track = function(event, fn){
  ...
  this.lock(key, function(err){
    ...
    var jobKey = [self.settings.appId, 'jobs', id].join(':');
...
/**
 * Make a request to add to an existing Bulk Event Job and then unlock
 *
 * @api private
 * @param {String} jobId
 * @param {String} key
...
Intercom.prototype.addToExisting = function(jobId, track, key, fn){
  var self = this;
  var data = mapper.track(track);
  data.job = { id: jobId };
  ...
  return this
    ...
    .end(self.handle(function(err, res){
      ...
      if (err) return self.createNewJob(track, key, fn);
      self.unlock(key, function(){
        return fn(err, res);
      });
    }));
...
/**
 * Make a request to create a new Bulk Event Job
...
Intercom.prototype.createNewJob = function(track, key, fn){
  var self = this
  var id = track.userId() || track.email();
  var jobKey = [this.settings.appId, 'jobs', id].join(':');
