...
exports.identify = function(msg){
  ...
  if (is.array(companies)) {
    ret.companies = companies.map(formatCompany).filter(is.object);
    if (!ret.companies.length) delete ret.companies;
  }
...
function formatCompany(company){
  ...
  if (!is.object(company)) return;

  var ret = {};
  if (company.name) ret.name = company.name;
